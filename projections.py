...
class WarpingBackProjection(ConvexProjection):
    def __init__(self, projection_matrix):
        ...
        org_bp_matrix= np.transpose(self.A) @ np.linalg.inv(self.A @ np.transpose(self.A)) @ self.A
    ...
    def evaluate(self, low_dim_vector):
        ...
        for i in range(n):
            ...
            # Doing back projection for the points which are outside of the space spanned by A
            if suitable_for_bp:
